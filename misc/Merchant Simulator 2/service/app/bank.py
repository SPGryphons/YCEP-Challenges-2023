...
FREE_DEBT_INTEREST_RATE = 1.0 # This is definitely fair...
PREMIUM_DEBT_INTEREST_RATE = 0.025
BASE_MIN_LOAN = 5000
...
class Bank:
  ...
  def collect_interest(self, weeks: int = 1):
    ...
    if self.game.mts.has_purchased("premium_interest_rate"):
      DEBT_INTEREST_RATE = PREMIUM_DEBT_INTEREST_RATE
    else:
      DEBT_INTEREST_RATE = FREE_DEBT_INTEREST_RATE
    if self.player_debt != 0:
      self.player_debt = int(self.player_debt * (1 + DEBT_INTEREST_RATE) ** weeks) # Compound interest
